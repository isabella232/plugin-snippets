...
async function runEveryDay({ config, global }) {
    ...
    for (let tag of newTags) {
        ...
        const createAnnotationRes = await fetchWithRetry(`${global.posthogHost}/api/annotation/`, {
            headers: {
...
function getTagDate(tag) {
    return (
        tag.tagger ? tag.tagger.date : 
        tag.author ? tag.author.date : 
        tag.committer ? tag.committer.date : 
        null
    )
}